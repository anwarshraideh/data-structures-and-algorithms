...
// Solution code here...
  var longeStr = arr.reduce((a, b) => a.length > b.length ? a : b , '');

  return arr.indexOf(longeStr);
...
const firstLetters = (arr) => {
  // Solution code here...
  let firstLettersArr = arr.map(element => element.substring(0, 1));
...
const findHappiness = (arr) => {
  ...
  let newArray = arr.filter(element => element.includes(':)'));
  return newArray;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(element => element.substring(1, 4) + element.substring(6, 9) + element.substring(10, 14));
