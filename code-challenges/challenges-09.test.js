...
function transformToLis(obj){
  ...
  let newArray = keys.map((el, index) => `<li>${el}: ${values[index]}</li>`);
  return newArray ;
}
...
const addValues = (arr) => {
  ...
  let NewArray = arr.reduce((addVal,value)=>{
    addVal += value;
    return addVal ;
  },0);
  return NewArray;
...
const addPurchases = (arr) => {
  // Solution code here...
  let NewArray = arr.reduce((addPurchas, value) => {
    addPurchas += value.purchasePrice;
    return addPurchas;
  }, 0);
  return NewArray;
...
const reversedString = (str) => {
  ...
  let newaArray = str.split('');
  let reversStr = newaArray.reduce((accumulator, value) => {
