...
const typeNum = (arr) => {
  // Solution code here...
  let numbers = arr.filter(n => Number(n) === n);
  return numbers;
};
...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let voulesArr = arr.filter(element =>
  {
    return /[aeiou]/g.test(element);
  });
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  return arr.filter((item) => { return !forbiddenValues.includes(item); });
};
