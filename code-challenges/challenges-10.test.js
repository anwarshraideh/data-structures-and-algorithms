...
const getNames = (arr) => {
  ...
  let reversedArray = arr.map(element=>{
...
const totalSum = (input) => {
  ...
  let sum = 0;
  input.map(arr => arr.map(element => sum += element));
  return sum;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArray = [];
  input.forEach(arr => {
    newArray.push(arr.filter(num => typeof num === 'number' && num % 5 === 0).map(num => Math.pow(2, num)));
  });
  return newArray;
...
let findMaleAndFemale = (data) => {
  ...
  let final = newArray.map(element => element.name);
